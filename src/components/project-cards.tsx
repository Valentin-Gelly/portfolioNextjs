...
export default function ProjectCard({ name, description, links }: ProjectCardProps) {
    return (
        <div
            className="max-w-xl h-[600px] bg-white border border-gray-200 rounded-xl shadow-sm dark:bg-gray-800 dark:border-gray-700 hover:shadow-lg transition-shadow duration-300 ease-in-out">
            <div className="relative overflow-hidden rounded-t-lg group">
                ...
                />
                <div
                    className="absolute inset-0 bg-gradient-to-b from-transparent via-white/50 to-white dark:via-gray-800/50 dark:to-gray-800 pointer-events-none">
                </div>
            </div>
            <div className="p-5 flex flex-col justify-between h-1/2">
                ...
                <p className="mb-3 text-xl text-gray-700 dark:text-gray-400">{description}</p>
                <div className="flex flex-wrap gap-2">
                    {links.map((link, index) => (
                        <a
                            key={index}
                            href={link.href}
                            className="inline-flex items-center px-3 py-2 text-sm font-medium text-center text-white focus:ring-4 focus:outline-none focus:ring-blue-300 dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800"
                        >
                            {link.type === "github" && (
                                <Image
                                    className=" mr-2"
                            ...
                            {link.type === "demo" && (
                                <svg width={40} height={40} viewBox="0 0 1024 1024" transform="scale(1.2)">
